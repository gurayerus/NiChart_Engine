...
def select_spare_sample(in_csv, in_sample, in_vars, out_csv):
    ...
    Select data based on list of samples and list of variables
     - in_sample: MRID+Target or MRID+Age
    ...
    # If sample has column Age instead of Target drop Age from data and rename Age to Target
    # Specific rule for Brain Age models
    if 'Age' in dfs.columns:
        df_out = df_out.drop(columns = 'Age')
        dfs = dfs.rename(columns = {'Age':'Target'})
