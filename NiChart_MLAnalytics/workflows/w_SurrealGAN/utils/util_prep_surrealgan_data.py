...
def prep_surrealgan_data(in_csv, out_demog_csv, out_roi_csv):
    ...
    # Select rois
    selROIs = [ name for name in df.columns if (name[0:14]=='MUSE_' and int(name[14:])<300) and int(name[14:]) not in [4,11,49,50,51,52]] # selected single ROIs from GM and WM    
    ...
    for i in range(len(selROIs)//2):
        mROIs.append('MUSE_' + selROIs[i*2][14:] + '_' + selROIs[i*2+1][14:])
    for i in range(len(selROIs)//2):
        df['MUSE_' + selROIs[i*2][14:] + '_'+selROIs[i*2+1][14:]] = df[selROIs[i*2]] + df[selROIs[i*2+1]]
