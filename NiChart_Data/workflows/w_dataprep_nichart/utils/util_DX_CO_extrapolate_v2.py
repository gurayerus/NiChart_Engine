...
def col_name_conversion(df, diagnosis, diagnosis_value_mapping, var_type = 'DX', output_path = None):
    '''
    ...
    if output_path is not None:
        ...
        print(f'Final Conslidated File has successfully stored at: {output_path}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i1','--input_combined_path', type = str, help = 'Clinical MRI combined Data input path', default = None)
    parser.add_argument('-i2','--input_clinical_path', type = str, help = 'Clean Clinical Data input path', default = None)
    parser.add_argument('-i3','--input_flag_path', type = str, help = 'Flag Data input path', default = None)
    ...
    input_combined_path = args.input_combined_path
    input_clinical_path = args.input_clinical_path
    input_flag_path = args.input_flag_path
    ...
    print('Input combined Path: ', input_combined_path)
    print('Input clinical Path: ', input_clinical_path)
    print('Input input Flag Path : ', input_flag_path)
    ...
    if os.path.exists( input_combined_path ):
        df_combined = pd.read_csv( input_combined_path )
    else:
        raise Exception("Combined files are not exist in the path, please use a valid path.")
    
    if os.path.exists(input_clinical_path):
        df_clin = pd.read_csv(input_clinical_path)
    else:
        raise Exception("Clinical files are not exist in the path, please use a valid path.")
    
    if os.path.exists(input_flag_path):
        with open( input_flag_path, 'rb') as f:
            flag = pickle.load(f)        
    else:
        raise Exception("The Flag fliles are not exist in the path, please use a valid path.")
    
    with open(map_dic_path, 'r') as json_file:
